...
def interset(b1,b2,size):
    ...
    if b2 is None:
        return True
    return in_bbox(b1,(b2[0],b2[1]),size) or in_bbox(b1,(b2[2],b2[1]),size) or in_bbox(b1,(b2[2],b2[3]),size) or in_bbox(b1,(b2[0],b2[3]),size)
...
class Parser(object):
    ...
    def parse(self):
        ...
        img = np.ones((page_bbox[3], page_bbox[2], 3), np.uint8) * 255
        for bbox in self.lines:
            if (bbox[2] - bbox[0]) > 50 or (bbox[3] - bbox[1]) > 50:
                img = cv2.line(img, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 0), thickness=1)
        for bbox in self.rects:
            if bbox[2] - bbox[0] > 50 or (bbox[3] - bbox[1]) > 50:
                img = cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 0), thickness=1)
    ...
    def subtable(self,bbox,name,v=10,h=15,it=0):
    ...
    def table_one(self, coi, cells):
        ...
        ncells = self.subtable(self.get_bbox(ncells,0,10),'first_table')
    ...
    def table_second(self, coi, cells):
        ...
        def _get_limits(i1, i2):
            ...
            for i, row in enumerate(cells[i1:i2]):
                ...
                if name=='':
                    print(self.get_latters(name_bb))
